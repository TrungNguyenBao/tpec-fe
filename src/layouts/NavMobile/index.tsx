...
const NavMobile = () => {
  const { globalData, showNavMobile, setShowNavMobile } = useGlobalContext();
  ...
  return (
    <ul
      className={`block lg:hidden text-base font-semibold text-colorcs-fff absolute z-50 duration-200 ${
            ...
            <ul className="text-colorcs-fff ">
              {item?.menuChildren?.articles?.data?.map((itemMC) => (
                <li key={"menu-m-link-" + itemMC?.id}>
                  <Link href={`/${itemMC?.attributes?.slug}-${itemMC?.id}`}>
                    <a
